...
function edge_resp_and_custom_css() {
	...
	<?php  }
	if (($edge_settings['edge_slider_content_bg_color']=='on' ) || ($edge_settings['edge_disable_big_letter']=='off') || ($header_display=='header_logo')){
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'edge_slider_image');
				...
				$remove_link = $edge_settings['edge_slider_link'];
					...
					if ($excerpt != '') {
						...
						if($edge_settings['edge_crop_excerpt_length'] ==1){
							$edge_page_sliders_display .= '<h3>'.wp_strip_all_tags($excerpt).' </h3></div><!-- end .slider-text -->';
						} else {
							$edge_page_sliders_display .= '<h3>'.wp_strip_all_tags(get_the_excerpt()).' </h3></div><!-- end .slider-text -->';
						}
					}
...
function edge_scripts() {
	...
	wp_enqueue_script('edge-navigation', get_template_directory_uri().'/js/navigation.js', array('jquery'), false, true);
	// Load the html5 shiv.
